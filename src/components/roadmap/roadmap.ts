...
export class Roadmap extends DivElement {
  constructor(readonly data: RoadmapData[]) {
    super({ 
      className: "flex flex-col p-6 scrollbar-hide h-[600px] w-full max-w-7xl mx-auto",
  ...
  private createSection(section: RoadmapData): DivElement {
    const container = new DivElement({ 
      className: "flex-shrink-0 snap-start min-w-[350px] max-w-[300px] border rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow duration-300 overflow-y-auto max-h-[600px] roadmap-section scrollbar-hide"
    });
  ...
  private createSectionHeader(section: RoadmapData): DivElement {
    ...
    header.appendChild(new SpanElement({ 
...
class CardElement extends DivElement {
  ...
  private createHeader(): DivElement {
    ...
    header.appendChild(new DivElement({
  ...
  private createBody(): DivElement {
    ...
    this.card.body.forEach(item => {
      container.appendChild(new DivElement({
  ...
  private createTagsContainer(): DivElement {
    ...
    this.card.tags!.forEach(tag => {
      container.appendChild(new SpanElement({
