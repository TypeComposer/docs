...
export class BorderPanelView extends BaseView {
	constructor() {
		super();
		this.append(new SpanElement({ innerText: "TypeComposer ∙ BorderPanel" }));
		this.append(new H1Element({ innerText: "Overview" }));
		this.append(new ParagraphElement({ innerText: "The BorderPanel is a layout component in TypeComposer that divides the screen into distinct regions: top, left, right, bottom, and center. It comes with two layout variants ('primary' and 'secondary'), which define the arrangement of these regions." }));
		...
		this.append(new DivElement({
			...
			gap: "10px",
			children: [
				new ImageElement({ src: "/images/border-pane-primary.png", alt: "Primary Layout", className: "w-1/2", maxWidth: "400px" }),
				new ImageElement({ src: "/images/border-pane-secondary.png", alt: "Secondary Layout", className: "w-1/2", maxWidth: "400px" })
			]
		}));
...
export class AppPage extends BorderPanel {
	...
	bottom: new Footer(),
		...
		this.append(new ParagraphElement({ innerText: "By default, the BorderPanel uses the 'primary' variant and arranges its regions accordingly." }));
		...
		this.append(new H1Element({ innerText: "7. When to Use BorderPanel" }));
		...
		this.append(new ParagraphElement({ innerText: "The BorderPanel is a powerful layout component for creating structured, multi-region layouts in TypeComposer. With its two layout variants ('primary' and 'secondary') and easy-to-customize properties, it provides a flexible solution for building complex UI layouts with minimal code." }));
