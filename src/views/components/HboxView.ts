...
export class HboxView extends BaseView {
	constructor() {
		super();
		this.append(new SpanElement({ innerText: "TypeComposer ∙ HBox" }));
		this.append(new H1Element({ innerText: "Overview" }));
		this.append(new ParagraphElement({ innerText: "The HBox is a container component in TypeComposer designed to arrange its child elements horizontally. It comes with built-in styles for horizontal alignment and is highly customizable through its properties." }));
		...
		this.append(new CodeComponent({
			code: `import { HBox, DivElement } from "typecomposer";
...
export default ExampleHBox;`
	...
	new DivElement({ text: "Right Item", padding: "10px" }),
		...
		this.append(new ParagraphElement({ innerText: "By default, the HBox:" }));
		...
		this.append(new H1Element({ innerText: "6. When to Use HBox" }));
