...
export class VboxView extends BaseView {
	constructor() {
		super();
		this.append(new SpanElement({ innerText: "TypeComposer ∙ VBox" }));
		this.append(new H1Element({ innerText: "Overview" }));
		this.append(new ParagraphElement({ innerText: "The VBox is a container component in TypeComposer designed to stack its child elements vertically. It comes with built-in styles for vertical alignment and is highly customizable through its properties." }));
		...
		this.append(new CodeComponent({
			code: `import { VBox, DivElement } from "typecomposer";
...
export default ExampleVBox;`
	...
	new DivElement({ text: "Bottom Item", padding: "10px" }),
		...
		this.append(new ParagraphElement({ innerText: "By default, the VBox:" }));
		...
		this.append(new H1Element({ innerText: "6. When to Use VBox" }));
